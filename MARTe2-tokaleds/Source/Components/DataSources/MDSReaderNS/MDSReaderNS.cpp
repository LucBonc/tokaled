...
std::cout << "MDS READER INIT" <<std::endl;
    ...
    if (ok) {
        ok = data.Read("RewindAt", tEnd);
        if (!ok) {
	    ok=true;
	    rewind=false;
            REPORT_ERROR(ErrorManagement::Information, "RewindAt not  specified");
        }
        else {
            rewind=true;
	    REPORT_ERROR(ErrorManagement::Information, "RewindAt %f sec specified",tEnd);
        }
    }
