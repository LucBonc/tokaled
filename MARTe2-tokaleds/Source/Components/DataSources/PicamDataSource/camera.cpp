...
CCamera* StartCamera(int width, int height, int framerate, int num_levels, bool do_argb_conversion,bool startCapture)
{
	...
	GCamera = new CCamera();
	if(!GCamera->Init(width,height,framerate,num_levels,do_argb_conversion, startCapture))
	{
		//failed so clean up
...
bool CCamera::Init(int width, int height, int framerate, int num_levels, bool do_argb_conversion,bool start)
{
	...
	//begin capture
		...
		printf("Remeber to call StartCapture() when  you want to start Acquisition\n");
	}
